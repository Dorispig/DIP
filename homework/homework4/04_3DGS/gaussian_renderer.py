...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        ### FILL:
        ### J_proj = ...
        x, y, z = torch.unbind(cam_points, dim=-1)  # (N,)
        z2 = (z + 1e-4) ** 2  # (N,)
        fx = K[0, 0]  # 焦距 fx
        fy = K[1, 1]  # 焦距 fy
    ...
    def compute_gaussian_values(
        ...
        #covs2D = covs2D + eps * torch.eye(2, device='cuda:0').unsqueeze(0)
        
        # Compute determinant for normalization
        ### FILL: compute the gaussian values
        ### gaussian = ... ## (N, H, W)
        # Compute determinant for normalization
        det_covs2D = torch.det(covs2D) + eps  # (N, )
        
        # Compute inverse of covariance matrix
        inv_covs2D = torch.inverse(covs2D)  # (N, 2, 2)
        
        # Compute the quad form
        quad_form = -0.5*torch.einsum('nhwi,nhwi->nhw',dx,torch.einsum('njk,nhwk -> nhwj',inv_covs2D,dx))
        # Compute the normalization term
        norm_term = 1.0 / (2 * torch.pi * torch.sqrt(det_covs2D)).unsqueeze(1).unsqueeze(2)  # (N, 1, 1)
        
        # Compute the gaussian values
        gaussian = norm_term * torch.exp(quad_form)  # (N, H, W)
