...
def update_texure(pyrender_mesh, tex_image):
    ...
    # 修改纹理
    texture.source = tex_image.copy()  # R, G, B通道设置为255，A通道保持不变
    # # 获取纹理的源数据
    # texture_source = texture.source
    #
    # # 如果源数据是NumPy数组，可以直接使用
...
def tex_Mask(s: Myscene, I, T_star, mask_range=16):
    ...
    proj_map = None
    print('得到projection_map2')

    scene = generate_scene(s.init_pyrender_mesh)
    add_light(scene, s.light1_type, s.light1_pose)
    add_light(scene, s.light2_type, s.light2_pose)
    add_light(scene, s.light3_type, s.light3_pose)
    add_light(scene, s.light4_type, s.light4_pose)
    ...
    renderer = pyrender.OffscreenRenderer(H, W)
    # 渲染场景
    init, _ = renderer.render(scene)
    init = (init * 1).astype(np.uint8)
    alpha = np.abs(init - I)
    alpha = np.sum(alpha, axis=2)
    alpha = (alpha < 50).astype(bool)
    # alpha = np.zeros((h, w))
    # for i in range(h):
    #     for j in range(w):
    #         if proj_map[j][h-1 - i][0]+proj_map[j][h-1 - i][1] == 0:
    #             continue
    #         # print('proj_map[i][j]:',proj_map[i][j])
    #         u = int(proj_map[j][h-1 - i][0] * tex_h)
    #         v = int(proj_map[j][h-1 - i][1] * tex_w)
    #         if np.sum(T_star[u][v][:3]) == 0:
    #             alpha[i][j] = 1
    # # 纹理掩码，无纹理区域设置为1
    # alpha = alpha.T
    tex_mask = mask * alpha
    ...
    return proj_map, expanse_tex_mask, mask, alpha

def M_ij(si: Myscene, Ii, sj: Myscene, Ij, T_star, mask_range=16):
    proj_mapi, Mi, maski, alphai = tex_Mask(si, Ii, T_star, mask_range)
    proj_mapj, Mj, maskj, alphaj = tex_Mask(sj, Ij, T_star, mask_range)
    Mij = np.hstack((Mi,Mj))
    maskij = np.hstack((maski,maskj))
    alphaij = np.hstack((alphai,alphaj))
    return proj_mapi, Mi, maski, alphai, proj_mapj, Mj, maskj, alphaj, Mij, maskij, alphaij
